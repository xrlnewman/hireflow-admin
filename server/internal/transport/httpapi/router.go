...
import (
	...
	"github.com/xrlnewman/hireflow-admin/server/internal/app/dispatch"
	"github.com/xrlnewman/hireflow-admin/server/internal/app/inventory"
	orderapp "github.com/xrlnewman/hireflow-admin/server/internal/app/order"
	"github.com/xrlnewman/hireflow-admin/server/internal/config"
	"github.com/xrlnewman/hireflow-admin/server/internal/domain"
	platformauth "github.com/xrlnewman/hireflow-admin/server/internal/platform/auth"
	"github.com/xrlnewman/hireflow-admin/server/internal/platform/cache"
...
func NewRouterWithDeps(cfg config.Config, st *store.MemoryStore, deps Dependencies) *gin.Engine {
	...
	// 招聘领域别名：候选人筛选与面试结果复用同一幂等状态服务。
	protected.GET("/recruiting/candidates", s.stockProducts)
	protected.GET("/recruiting/reminders", s.stockAlerts)
	protected.POST("/recruiting/candidates/:id/screen", s.stockReceive)
	protected.GET("/recruiting/interviews", s.stockSales)
	protected.POST("/recruiting/interviews/:id/complete", s.stockShip)
...
func seed(st *store.MemoryStore) {
	...
	st.SeedUser(store.User{ID: "user-demo", Phone: "13800000000", PasswordHash: string(hash), Name: "演示候选人", Role: "customer"})
