...
func (s *Server) stockDashboard(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取人才概览失败", nil)
		return
	}
...
func (s *Server) stockWarehouses(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取招聘渠道失败", nil)
		return
	}
...
func (s *Server) stockProducts(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取候选人失败", nil)
		return
	}
...
func (s *Server) stockAlerts(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取招聘提醒失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockPurchases(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取候选人单失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockSales(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取面试单失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockMovements(c *gin.Context) {
	...
	if err != nil {
		s.envelope(c, http.StatusInternalServerError, "读取招聘动态失败", nil)
		return
	}
	s.envelope(c, http.StatusOK, "ok", gin.H{"list": data, "total": total, "page": page, "pageSize": pageSize})
}
...
func (s *Server) stockMutationError(c *gin.Context, err error, fallback string) {
	...
	case errors.Is(err, inventory.ErrNotFound):
		s.envelope(c, http.StatusNotFound, "人才单据不存在", nil)
	case errors.Is(err, inventory.ErrAlreadyCompleted):
		s.envelope(c, http.StatusConflict, "人才单据已经完成", nil)
