...
var (
	ErrNotFound         = errors.New("人才单据不存在")
	ErrAlreadyCompleted = errors.New("人才单据已经完成")
...
func NewService() *Service {
	...
	return &Service{
		...
		products: []Product{
			{ID: "p-1001", SKU: "HF-1001", Name: "轻量通勤双肩包", Category: "箱包", Warehouse: "东城中心仓", Unit: "件", Stock: 126, MinStock: 40, Price: 169, Status: "normal"},
			{ID: "p-1002", SKU: "HF-1002", Name: "便携保温杯 480ml", Category: "家居", Warehouse: "东城中心仓", Unit: "件", Stock: 18, MinStock: 30, Price: 89, Status: "warning"},
			{ID: "p-1003", SKU: "HF-1003", Name: "有机棉基础 T 恤", Category: "服饰", Warehouse: "西郊备货仓", Unit: "件", Stock: 9, MinStock: 24, Price: 129, Status: "danger"},
			{ID: "p-1004", SKU: "HF-1004", Name: "桌面收纳盒套装", Category: "家居", Warehouse: "西郊备货仓", Unit: "套", Stock: 64, MinStock: 20, Price: 59, Status: "normal"},
		},
		purchases: []PurchaseOrder{
			{ID: "PO20260716001", Supplier: "清和供应链", Warehouse: "东城中心仓", Items: 3, Amount: 12800, Status: "待筛选", CreatedAt: now},
			{ID: "PO20260715008", Supplier: "织物工坊", Warehouse: "西郊备货仓", Items: 2, Amount: 7360, Status: "已入库", CreatedAt: now},
		},
		sales: []SalesOrder{
			{ID: "SO20260716032", Customer: "星河生活馆", Warehouse: "东城中心仓", Items: 8, Amount: 3280, Status: "待面试", CreatedAt: now},
...
func (s *Service) Dashboard(ctx context.Context) (Dashboard, error) {
	if err := ctx.Err(); err != nil {
		return Dashboard{}, fmt.Errorf("读取人才概览: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Warehouses(ctx context.Context) ([]Warehouse, error) {
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("读取招聘渠道: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Products(ctx context.Context, keyword string, page, pageSize int) ([]Product, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取候选人: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Alerts(ctx context.Context, page, pageSize int) ([]StockAlert, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取招聘提醒: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Purchases(ctx context.Context, page, pageSize int) ([]PurchaseOrder, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取候选人单: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Receive(ctx context.Context, id, idem string) (PurchaseOrder, error) {
	...
	for i := range s.purchases {
		...
		s.movements = append([]Movement{{ID: "MV" + time.Now().UTC().Format("20060102150405"), Product: "候选人导入", Warehouse: s.purchases[i].Warehouse, Type: "入库", Quantity: s.purchases[i].Items, Source: id, CreatedAt: time.Now().UTC().Format(time.RFC3339)}}, s.movements...)
...
func (s *Service) Sales(ctx context.Context, page, pageSize int) ([]SalesOrder, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取面试单: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Service) Ship(ctx context.Context, id, idem string) (SalesOrder, error) {
	...
	for i := range s.sales {
		...
		s.movements = append([]Movement{{ID: "MV" + time.Now().UTC().Format("20060102150406"), Product: "面试流程", Warehouse: s.sales[i].Warehouse, Type: "出库", Quantity: s.sales[i].Items, Source: id, CreatedAt: time.Now().UTC().Format(time.RFC3339)}}, s.movements...)
...
func (s *Service) Movements(ctx context.Context, page, pageSize int) ([]Movement, int, error) {
	if err := ctx.Err(); err != nil {
		return nil, 0, fmt.Errorf("读取招聘动态: %w", err)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
