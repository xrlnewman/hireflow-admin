...
export const demoSnapshot = Object.freeze({
  ...
  alerts: Object.freeze([{ productId: 'p-1002', sku: 'HF-1002', name: '便携保温杯 480ml', warehouse: '东城中心仓', stock: 18, minStock: 30, severity: 'warning' }, { productId: 'p-1003', sku: 'HF-1003', name: '有机棉基础 T 恤', warehouse: '西郊备货仓', stock: 9, minStock: 24, severity: 'danger' }]),
  products: Object.freeze([{ id: 'p-1001', sku: 'HF-1001', name: '轻量通勤双肩包', category: '箱包', warehouse: '东城中心仓', stock: 126, minStock: 40, price: 169, status: 'normal' }, { id: 'p-1002', sku: 'HF-1002', name: '便携保温杯 480ml', category: '家居', warehouse: '东城中心仓', stock: 18, minStock: 30, price: 89, status: 'warning' }, { id: 'p-1003', sku: 'HF-1003', name: '有机棉基础 T 恤', category: '服饰', warehouse: '西郊备货仓', stock: 9, minStock: 24, price: 129, status: 'danger' }, { id: 'p-1004', sku: 'HF-1004', name: '桌面收纳盒套装', category: '家居', warehouse: '西郊备货仓', stock: 64, minStock: 20, price: 59, status: 'normal' }]),
  purchases: Object.freeze([{ id: 'PO20260716001', supplier: '清和供应链', warehouse: '东城中心仓', items: 3, amount: 12800, status: '待筛选' }, { id: 'PO20260715008', supplier: '织物工坊', warehouse: '西郊备货仓', items: 2, amount: 7360, status: '已入库' }]),
  sales: Object.freeze([{ id: 'SO20260716032', customer: '星河生活馆', warehouse: '东城中心仓', items: 8, amount: 3280, status: '待面试' }, { id: 'SO20260716031', customer: '林先生', warehouse: '西郊备货仓', items: 2, amount: 258, status: '已完成' }]),
...
export function createApiClient({ baseUrl = envApiBaseUrl(), token: initialToken = '', storage = typeof globalThis.localStorage !== 'undefined' ? globalThis.localStorage : null, fetchImpl = globalThis.fetch } = {}) {
  let token = initialToken || storage?.getItem?.('hireflow_access_token') || '';
  ...
  async function fallback(name, value, action) { try { return { source: 'api', data: await action() }; } catch (error) { console.warn(`[HireFlow] ${name} 回退演示数据`, error); return { source: 'demo', data: value }; } }
  async function login(credentials) { const data = await request('/auth/login', { method: 'POST', body: JSON.stringify(credentials) }); if (!data?.accessToken) throw new Error('登录响应缺少访问令牌'); token = data.accessToken; storage?.setItem?.('hireflow_access_token', token); return data; }
  async function logout() { if (baseUrl && token) { try { await request('/auth/logout', { method: 'POST' }); } catch (error) { console.warn('[HireFlow] 退出接口失败', error); } } token = ''; storage?.removeItem?.('hireflow_access_token'); }
  ...
  async function mutate(path, id, name) { return fallback(name, (demoSnapshot[name] || []).find((item) => item.id === id) || {}, () => request(`${path}/${id}/${name === 'purchases' ? 'receive' : 'ship'}`, { method: 'POST', headers: { 'Idempotency-Key': `hireflow-${name}-${id}` }, body: '{}' })); }
